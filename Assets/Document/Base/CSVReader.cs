...
public class CSVReader
{
    ...
    private System.String[,] _arr_grid;

    public CSVReader()
    {
    }

    public CSVReader(System.String[,] grid)
    {
        _arr_grid = grid;
    }
    ...
    public void get(int row, ref float val)
    {
        ...
        {
            val = 0f;
            Debug.LogWarning($"[CSVReader] 공백 또는 빈 값 → 0f 처리 (row {row}, col {cur_col - 1})");
            return;
        }

        // 인코딩 문제로 앞에 BOM이 붙어있는 경우 제거
        s = s.Trim().Trim('\uFEFF');

        if (!float.TryParse(s, out val))
        {
            Debug.LogWarning($"[CSVReader] float 파싱 실패: '{s}' → row {row}, col {cur_col - 1} → 기본값 0f");
            val = 0f;
        }
    }
